...
use CAO\Controllers\ArtikelController;
use CAO\Core\Filesystem;

require_once __DIR__."/../api/cao.php";

if (empty($_REQUEST['type'])) {
	die("Type was not defined");
}
...
class cao_ui
{
			...
			case 'settings':
				...
				switch ($_REQUEST['action']) {
					case 'Bill':
						...
						echo json_encode(array("html" => $html, "request" => $_REQUEST));
						exit;
						break;
	...
	public function HandleAjax() {
		...
		if (!empty($_REQUEST['check_artikel'])) {
			...
			<table class="table-fill">
				...
				<tbody class="table-hover">
					<?php foreach ($search as $key => $adr): ?>
						...
						$data_id = base64_encode(encryptIt(json_encode(array(
							"num"			=>	$KUNNUM,
			...
			echo(json_encode(array("bill"=>$status)));
			exit();
